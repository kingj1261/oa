...
CREATE TABLE OA_MESSAGE(
  ...
  customer_id             VARCHAR(32) NOT NULL COMMENT '消息接收人编号',
  customer_name           VARCHAR(32) NOT NULL COMMENT  '消息接收人名称',
  read_status             VARCHAR(8) NOT NULL COMMENT  '阅读状态 R-已读 NR-未读',
  message_type            VARCHAR(8) NOT NULL COMMENT '消息类型 SYSTEM-系统消息 BUSINESS-系统消息 OA-OA消息 ESHOP-电商消息',
...
(
  id                      INT PRIMARY KEY NOT NULL AUTO_INCREMENT COMMENT '逻辑主键',
  message_id              INT NOT NULL COMMENT '消息表id',
  remind_type             VARCHAR(32) NOT NULL COMMENT '提醒类型 BUSINESS-业务提醒 OUT_TIME-到期提醒',
  gmt_create              DATETIME NOT NULL COMMENT '创建日期',
  gmt_modified            DATETIME NOT NULL COMMENT '最后修改日期'
)COMMENT '提醒表';
