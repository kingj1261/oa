...
function blueBtn(){
    var s = [];
    var finanmoney = $("#finanmoney");
    var finatestyzmzz = $("finatestyzmzz");

    if(finanmoney.val() == "" || isNaN(finanmoney.val() || finanmoney.val() <0)){
        s.push(finanmoney);
    }
    if(finatestyzmzz.val() == "" || isNaN(finatestyzmzz.val() || finatestyzmzz.val() <0)){
        s.push(finatestyzmzz);
    }
    //处理提示语
    if(s.length>0) $(".tiptext").show();
    else {
    ...
    //处理不规则的边框样式
    $("#finanmoney,#finatestyzmzz").css("border","1px solid #c2c2c2");
